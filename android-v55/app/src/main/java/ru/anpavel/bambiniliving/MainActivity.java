...
public class MainActivity extends Activity {
    ...
    private volatile String progressStage = "START";
    private volatile String progressId = "";
    private volatile String progressFile = "";
    private volatile long progressDone = 0;
    ...
    private JSONObject statusJson() {
        ...
        try {
            ...
            for (String id : WARM_IDS) if (isWarmReady(id)) ready.put(id);
            int pct = progressTotal > 0 ? (int)Math.min(100, (progressDone * 100L) / progressTotal) : 0;
            o.put("stage", progressStage);
            o.put("id", progressId);
            o.put("file", progressFile);
            o.put("done", progressDone);
            o.put("total", progressTotal);
            o.put("percent", pct);
            o.put("ready", ready);
            o.put("error", progressError);
    ...
    private void publishProgress() {
        if (web == null) return;
        String js = "window.onWarmProgress&&window.onWarmProgress(" + JSONObject.quote(statusJson().toString()) + ")";
        main.post(() -> {
            try { web.evaluateJavascript(js, null); } catch (Exception ignored) {}
        });
    }

    private void setProgress(String stage, String id, String file, long done, long total, String error) {
        progressStage = stage;
        progressId = id == null ? "" : id;
        progressFile = file == null ? "" : file;
        progressDone = done;
        progressTotal = total;
        progressError = error == null ? "" : error;
        publishProgress();
    }
    ...
    private void fillWarmPool() {
        for (String id : WARM_IDS) {
            if (isWarmReady(id)) {
                setProgress("READY_EXISTING", id, "", 1, 1, "");
                continue;
            }
            ...
            try {
                ...
                setProgress("MANIFEST", id, "index.m3u8", 0, 1, "");
                String playlist = fetchText(TEST_ORIGIN + "/hlsav/" + id + "/index.m3u8");

                List<String> files = new ArrayList<>();
                try {
                    long posterLen = contentLength(TEST_ORIGIN + "/hlsav/" + id + "/poster.jpg");
                    if (posterLen > 0) files.add("poster.jpg");
                } catch (Exception ignored) {}
                ...
                long total = playlist.getBytes(StandardCharsets.UTF_8).length;
                for (String name : files) {
                    long n = contentLength(TEST_ORIGIN + "/hlsav/" + id + "/" + name);
                    if (n > 0) total += n;
                }

                long[] done = {0};
                writeBytes(new File(tmp, "index.m3u8"), playlist.getBytes(StandardCharsets.UTF_8));
                done[0] += playlist.getBytes(StandardCharsets.UTF_8).length;
                setProgress("DOWNLOADING", id, "index.m3u8", done[0], total, "");

                for (String name : files) {
                    String url = TEST_ORIGIN + "/hlsav/" + id + "/" + name;
                    downloadToFile(url, new File(tmp, name), id, name, done, total);
                }

                writeBytes(new File(tmp, ".complete"),
                ...
                setProgress("READY", id, "", total, total, "");
                main.post(() -> {
                    try {
                        web.evaluateJavascript("window.onWarmUpdate&&window.onWarmUpdate()", null);
                    } catch (Exception ignored) {}
                });
            } catch (Exception e) {
                deleteTree(tmp);
                String msg = e.getClass().getSimpleName() + ": " + String.valueOf(e.getMessage());
                setProgress("ERROR", id, progressFile, progressDone, progressTotal, msg);
                try { Thread.sleep(2500); } catch (InterruptedException ignored) {}
            }
        }

        setProgress("POOL_COMPLETE", "", "", 1, 1, progressError);
    }

    private void downloadToFile(String url, File file, String id, String name, long[] done, long total) throws Exception {
        HttpURLConnection c = open(url, "GET");
        int code = c.getResponseCode();
        if (code < 200 || code >= 300) {
            c.disconnect();
            throw new Exception("HTTP " + code + " " + name);
        }
        ...
        try (InputStream in = c.getInputStream(); FileOutputStream out = new FileOutputStream(part)) {
            ...
            while ((n = in.read(b)) != -1) {
                ...
                if (now - lastPublish > 150) {
                    setProgress("DOWNLOADING", id, name, done[0], total, "");
                    lastPublish = now;
                }
            }
        } finally {
        ...
        if (!part.renameTo(file)) throw new Exception("rename failed " + name);
        setProgress("DOWNLOADING", id, name, done[0], total, "");
    }
    ...
    private static HttpURLConnection open(String url, String method) throws Exception {
        ...
        c.setRequestProperty("User-Agent", "BambiniLiving/5.5-debug");
        return c;
    }
    ...
    private static long contentLength(String url) throws Exception {
        ...
        try {
            int code = c.getResponseCode();
            if (code < 200 || code >= 300) throw new Exception("HEAD HTTP " + code);
